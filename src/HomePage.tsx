  ...
  const popups = [
    {
      ...
      icon: () => <Clock size={50} className="text-white" weight="bold" />
    },
    {
      name: "local",
      desc: "Local",
      component: () => <Local onCancel={setPopup} />,
      icon: () => <MapPin size={50} className="text-white" weight="bold" />
    },
    ...
    >
     ...
     <Fundo/>
      ...
      <div className='w-full max-h-1/2 md:h-full  flex flex-col md:w-1/2  overflow-hidden items-center relative'>
        ...
        <div className='w-[80%] h-[160px] -translate-y-[120px] text-white flex items-center justify-center flex-col fundo '>
      ...
      <div className='flex w-full mt-3 text-marinho -translate-y-[90px]'>
        {
          popups.map((item, index) =>
            ...
            >
              <div className='rounded-full bg-marinho text-white p-2'>
                {item.icon()}
              </div>
              {item.desc}
            </div>)
